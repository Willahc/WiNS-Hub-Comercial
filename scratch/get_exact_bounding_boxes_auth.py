...
def get_boxes_auth():
    ...
    with sync_playwright() as p:
        ...
        for res in resolutions:
            context = browser.new_context(viewport={"width": res["w"], "height": res["h"]}, ignore_https_errors=True)
            page = context.new_page()

            # Set dev token bypass if needed or wait for page mount
            page.goto(url, wait_until="networkidle")
            page.wait_for_timeout(2500)

            res_boxes = page.evaluate("""() => {
                const getB = (sel) => {
                ...
                return {
                    ...
                    topbar: getB('.topbar, header, .app-topbar'),
                    title: getB('.page-title, h1, .head-title'),
                    kpis_container: getB('.reconciled-kpi-grid, .kpi-grid'),
                    kpis_count: document.querySelectorAll('.reconciled-kpi-card, .kpi-card').length,
