...
BEGIN
  ...
  v_candidata := CASE
    ...
    WHEN v_validada AND v_valor >= 100000
         AND v_fonte_tipo IN ('OFICIAL', 'MANUAL') THEN 'PIPELINE'
    ELSE NULL
  END;

  -- Promoção monotônica: enriquecimento nunca rebaixa uma obra existente.
  v_nova := CASE
    WHEN CASE v_candidata
           WHEN 'PIPELINE' THEN 1 WHEN 'BRONZE' THEN 2
           WHEN 'PRATA' THEN 3 WHEN 'OURO' THEN 4 ELSE 0
         END
       > CASE v_classificacao_atual
           WHEN 'PIPELINE' THEN 1 WHEN 'BRONZE' THEN 2
           WHEN 'PRATA' THEN 3 WHEN 'OURO' THEN 4 ELSE 0
         END
      THEN v_candidata
  ...
  UPDATE engenharia.obras
     SET classificacao_computed = v_nova
   WHERE id = p_obra_id;
END;
$function$;
