...
describe('Radar de Sinais e Oportunidades Agro', () => {
  ...
  it('renderiza o funil visual com etapas e motivos de não promoção', () => {
    for (const value of ['Funil de sinais', 'Municípios avaliados', 'Candidatas', 'Validadas', 'Motivos de não promoção', 'Cobertura normal', 'Entidade não identificada', 'Contato indisponível', 'Decisor não comprovado', 'Regra indisponível']) {
      expect(page).toContain(value);
    }
  });
