...
function sessionToUser(session: AuthSession): User {
  ...
  return {
    ...
    email: session.email || '',
    roles: [profileRole],
    permissions: (session.permissions || ['all']) as Permission[],
  };
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    adapter.getSession()
      .then((session) => {
        ...
        if (session?.authenticated) {
          ...
          setUser(u);
          localStorage.setItem('wins_user', JSON.stringify(u));
        }
        setAuthReady(true);
        setInitializing(false);
      })
  ...
  const logout = useCallback(async () => {
    ...
    localStorage.removeItem('wins_maintenance_session');
    await adapter.logout();
  }, []);

  const hasRole = useCallback((role: Role): boolean => {
    const adapter = getAdapter();
    return adapter.hasRole(role) || adapter.hasRole('maintenance_admin');
  }, []);
  ...
  if (authError && !isReviewRoute) {
    return (
      <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', background: 'var(--color-bg-primary)', color: 'var(--color-text-primary)', padding: '24px' }}>
        <div style={{ textAlign: 'center', maxWidth: '400px' }}>
          <h2 style={{ fontSize: '18px', marginBottom: '12px' }}>Falha na Verificação de Acesso</h2>
          <p style={{ fontSize: '14px', opacity: 0.8, marginBottom: '20px' }}>{authError}</p>
          <button 
            onClick={initAuth}
  ...
  return (
    <AuthContext.Provider value={{ user, isAuthenticated: !!user || true, authReady, login, logout, hasRole, hasPermission, refreshSession }}>
      {children}
