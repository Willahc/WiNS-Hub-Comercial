...
type ApiPage<T>={items:T[];total:number;page:number;page_size:number;applied_filters:Record<string,unknown>;aggregates:ApiAggregates;meta:{total:number;source:string;lastUpdatedAt?:string;partialData:boolean}};
type ApiWork={source_id:string;nome:string;descricao?:string;descricao_publica?:string;municipio?:string;uf?:string;latitude?:number;longitude?:number;geoPrecision?:'exact'|'municipality'|'unknown';status?:string;fase?:string;setor?:string;valor_estimado?:number;investment_homologated?:boolean;data_publicacao?:string;data_anuncio?:string;cnpj?:string;empresa?:string;qualityScore:number;partial_data:boolean};
type ApiOpportunity={source_id:string;obra_id:string;cnpj:string;score:number;obra_nome:string;municipio?:string;uf?:string;fornecedor?:string;score_breakdown?:unknown};
type ApiSupplier={source_id:string;razao_social?:string;nome_fantasia?:string;cnae_descricao?:string;municipio?:string;uf?:string;matches_count?:number;source_updated_at?:string;qualityScore:number};
...
async function getRealDataset(options:EngineeringFilters&{page?:number;pageSize?:number}={}):Promise<EngineeringDataset>{
  ...
  const emptyAggregates:ApiAggregates={works_total:0,investment_records_count:0,investment_missing_count:0,investment_unhomologated_count:0,investment_status:'unavailable',financial_coverage_pct:0,municipality_count:0,company_count:0,missing_municipality_count:0,missing_company_count:0,status_counts:[],phase_counts:[],territories:[],opportunities:{opportunities_total:0,opportunities_active_total:0,matches_total:0,opportunities_linked:0,opportunities_active:0,matches_linked:0,works_with_opportunity:0,works_without_opportunity:0,active_rule:'score >= 70'}};
  ...
  const works:EngineeringWork[]=worksData.items.map((w: any,index)=>({
    ...
    companyIds:w.cnpj?[w.cnpj]:[],priority:index<20?'Alta':index<60?'Média':'Baixa',
    indicators:[{label:'Qualidade da fonte',value:`${w.qualityScore}%`},{label:'Precisão geográfica',value:w.geoPrecision==='municipality'?'Localização aproximada · centroide municipal':w.geoPrecision==='exact'?'Localização exata':'Localização ausente'}],
    events:[{date:w.data_publicacao?new Date(w.data_publicacao).toLocaleDateString('pt-BR'):'Sem data',title:'Registro capturado',detail:'Evento derivado da atualização da fonte real.'}],
  }));
  const companyMap=new Map<string,EngineeringCompany>();
  for(const s of suppliersData.items){companyMap.set(s.source_id,{id:s.source_id,name:s.razao_social||s.nome_fantasia||'Razão social não informada',tradeName:s.nome_fantasia||s.razao_social||'Nome não informado',cnpj:s.source_id,segment:s.cnae_descricao||'Segmento não informado',municipality:s.municipio||'Município não informado',state:s.uf||'—',founded:'Não informado',employees:0,revenue:0,score:s.qualityScore||0,workIds:[],opportunityIds:[],territories:s.uf?[s.uf]:[],links:[],history:[{date:s.source_updated_at?new Date(s.source_updated_at).toLocaleDateString('pt-BR'):'Sem data',title:'Cadastro legado atualizado',detail:'Fonte wins_agro.engenharia.fornecedores'}]})}
  for(const w of worksData.items){if(!w.cnpj)continue;const current=companyMap.get(w.cnpj);if(current){if(!current.workIds.includes(w.source_id))current.workIds.push(w.source_id);continue}companyMap.set(w.cnpj,{id:w.cnpj,name:w.empresa||`Empresa ${w.cnpj}`,tradeName:w.empresa||'Empresa',cnpj:w.cnpj,segment:w.setor||'Engenharia',municipality:w.municipio||'Não informado',state:w.uf||'—',founded:'Não informado',employees:0,revenue:0,score:w.qualityScore||50,workIds:[w.source_id],opportunityIds:[],territories:w.uf?[w.uf]:[],links:[{type:'Obra',name:w.nome}],history:[{date:w.data_publicacao?new Date(w.data_publicacao).toLocaleDateString('pt-BR'):'Sem data',title:'Vínculo identificado',detail:'Fonte real de Engenharia'}]})}
  const opportunities:EngineeringOpportunity[]=oppsData.items.filter(o=>works.some(w=>w.id===o.obra_id)).map(o=>({id:o.source_id,title:`Fornecedor relacionado a ${o.obra_nome}`,sector:works.find(w=>w.id===o.obra_id)?.sector||'Rodovias',municipality:o.municipio||'Não informado',estimatedValue:undefined as unknown as number,stage:Number(o.score)>=85?'Qualificação':'Identificada',companyId:o.cnpj,workId:o.obra_id,score:Math.round(Number(o.score)||0),justification:`Matching real baseado nos critérios registrados na fonte; score ${o.score}.`}));
  for(const o of opportunities){const c=companyMap.get(o.companyId);if(c&&!c.opportunityIds.includes(o.id))c.opportunityIds.push(o.id)}
  const errors:string[]=[];
  if(worksResult.status==='rejected')errors.push(`Obras: ${worksResult.reason?.message||'Falha na requisição'}`);
  if(opportunitiesResult.status==='rejected')errors.push(`Oportunidades: ${opportunitiesResult.reason?.message||'Falha na requisição'}`);
  if(suppliersResult.status==='rejected')errors.push(`Fornecedores: ${suppliersResult.reason?.message||'Falha na requisição'}`);
  const a=worksData.aggregates;
  return {works,companies:[...companyMap.values()],opportunities,meta:{source:worksData.meta.source||'API Onda 1',lastUpdatedAt:worksData.meta.lastUpdatedAt,partialData:worksData.meta.partialData,totalWorks:worksData.total,page:worksData.page,pageSize:worksData.page_size,appliedFilters:options,aggregates:{worksTotal:a.works_total,investmentTotal:a.investment_total!=null?Number(a.investment_total):undefined,investmentRecordsCount:a.investment_records_count,investmentMissingCount:a.investment_missing_count,investmentUnhomologatedCount:a.investment_unhomologated_count,investmentStatus:a.investment_status,financialCoveragePct:Number(a.financial_coverage_pct||0),municipalityCount:a.municipality_count,companyCount:a.company_count,missingMunicipalityCount:a.missing_municipality_count,missingCompanyCount:a.missing_company_count,statusCounts:a.status_counts||[],phaseCounts:a.phase_counts||[],territories:(a.territories||[]).map(t=>({municipality:t.municipio,uf:t.uf,worksCount:Number(t.works_count),investmentTotal:t.investment_total!=null?Number(t.investment_total):undefined,investmentUnavailableCount:Number(t.investment_unavailable_count),companyCount:Number(t.company_count),opportunityCount:Number(t.opportunity_count),updatedAt:t.updated_at})),opportunities:{opportunitiesTotal:Number(a.opportunities?.opportunities_total||0),opportunitiesActiveTotal:Number(a.opportunities?.opportunities_active_total||0),matchesTotal:Number(a.opportunities?.matches_total||0),opportunitiesLinked:Number(a.opportunities?.opportunities_linked||0),opportunitiesActive:Number(a.opportunities?.opportunities_active||0),matchesLinked:Number(a.opportunities?.matches_linked||0),worksWithOpportunity:Number(a.opportunities?.works_with_opportunity||0),worksWithoutOpportunity:Number(a.opportunities?.works_without_opportunity||0),activeRule:a.opportunities?.active_rule||'score >= 70'},lastUpdatedAt:a.last_updated_at},realData:true,error:errors.join('; ')||undefined}};
}
