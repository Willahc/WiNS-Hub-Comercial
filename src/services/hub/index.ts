...
async function loadHubData(filters?: HubFilters): Promise<HubDataset> {
  ...
  const uf = filters?.uf || undefined;
  const baseParams: Record<string, string | number | undefined> = { page_size: 100, sort: 'updated_desc' };
  ...
  const mapResponse = await httpClient.get<{ items: any[] }>('/visao-geral/mapa', { params: uf ? { uf } : {} });
  const events: HubEvent[] = (eventsResponse.data || []).map((e: any) => ({
    id: e.id, title: e.titulo || e.title || 'Evento de Engenharia', type: e.tipo || 'obra',
    territory: e.territorio || e.municipio || '—', status: e.status || 'ativo',
    value: Number(e.valor) || 0, severity: e.severidade || 'media',
    verticals: e.verticals || ['engenharia'], companyIds: e.companyIds || [],
    opportunityIds: e.opportunityIds || [], assetIds: e.assetIds || [],
    coordinates: e.coordinates || [-15.78, -47.92], date: e.data || e.date || '—',
    description: e.descricao || '', source: e.fonte || 'wins_agro.engenharia.obras',
  }));
  const companies: HubCompany[] = companiesResponse.data.items.map(c => ({
    id: c.source_id, name: c.razao_social || 'Sem razão social',
    tradeName: c.nome_fantasia || c.razao_social || 'Empresa', cnpj: c.source_id,
    segment: c.porte || 'Engenharia',
    territory: `${c.municipio || 'Município não informado'}, ${c.uf || '—'}`,
    verticals: ['engenharia'], score: Number(c.qualityScore) || 50,
    revenue: Number(c.capital_social) || 0, employees: 0, relationships: [],
  ...
  const opportunities: HubOpportunity[] = opportunitiesResponse.data.items.map(o => ({
    id: o.source_id, title: `Fornecedor relacionado a ${o.obra_nome}`,
    vertical: 'engenharia', territory: `${o.municipio || 'Não informado'}, ${o.uf || '—'}`,
    value: 0, stage: Number(o.score) >= 85 ? 'Qualificação' : 'Identificada',
    companyId: o.cnpj, eventId: '', score: Math.round(Number(o.score) || 0),
    justification: `Match real da Engenharia com score ${o.score}. Valor estimado por predição algorítmica.`,
    owner: 'Engenharia', nextStep: 'Validar oportunidade comercial',
  }));
  ...
  const add = (rows: any[], vertical: OverviewEntity['vertical'], kind: string, path: (r: any) => string) =>
    rows.forEach(r => {
      ...
      entities.push({
        ...
        vertical, kind, municipality, uf: ufVal,
        ...
        detailPath: path(r), latitude: 0, longitude: 0, geoPrecision: 'municipality',
      });
    });
  ...
  opportunities.slice(0, 25).forEach(o => entities.push({
    id: o.id, name: o.title, vertical: 'oportunidades', kind: 'Oportunidade',
    municipality: o.territory.split(',')[0],
    uf: o.territory.split(',')[1]?.trim() || '', territory: o.territory,
    source: 'Engenharia · matches_v2', detailPath: `/engenharia/oportunidades/${o.id}`,
    latitude: 0, longitude: 0, geoPrecision: 'municipality',
  }));
  entities = (mapResponse.data.items || []).map((x: any) => ({
    ...x,
  ...
  return {
    events, companies, opportunities, assets: [], territories: [],
    overview: {
      counts: {
        works: Number(worksResponse.data.meta?.total || workRows.length),
        companies: Number(companiesResponse.data.meta?.total || companies.length),
        ruralProperties: Number(ruralResponse.data.meta?.total || 0),
        carriers: Number(carrierResponse.data.meta?.total || 0),
        healthEstablishments: Number(healthResponse.data.meta?.total || 0),
        opportunities: Number(opportunitiesResponse.data.meta?.total || opportunities.length),
...
export const hubService = {
  ...
  async getCompany(id: string): Promise<HubCompany | undefined> {
    ...
    return {
      id: data.cnpj, name: data.legalName || 'Sem razão social',
      tradeName: data.tradeName || data.legalName || 'Empresa', cnpj: data.cnpj,
      segment: data.supplierProfile?.segment || 'Engenharia',
      territory: `${data.address?.municipality || 'Não informado'}, ${data.address?.state || '—'}`,
      verticals: ['engenharia'], score: Number(data.qualityScore) || 50,
      revenue: Number(data.capital) || 0, employees: 0,
