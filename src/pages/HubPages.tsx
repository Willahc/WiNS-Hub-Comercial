...
const StateError=({error,reset}:{error:string;reset:()=>void})=><div className="state-error"><AlertCircle size={34}/><h3>Falha no carregamento</h3><p>{error}</p><button className="btn btn-outline" onClick={reset}><RefreshCw size={14}/> Tentar novamente</button></div>;
...
export const GlobalOverview: React.FC = () => {
  ...
  const ufs = [...new Set(entities.map(x => x.uf).filter(Boolean))].sort();
          ...
          {relations.map(r => (
            <div className="reconciled-connection-card" key={r.title}>
              ...
              <div className="reconciled-connection-footer">
                <span>Vínculo homologado por chave explícita</span>
      ...
      {featured && (
        <section className="featured-event featured-event-compact" data-testid="featured-event">
          ...
          <div className="featured-facts">
            ...
            <span><b>CAPEX Homologado</b>{featured.value > 0 ? money(featured.value) : 'Valor não homologado'}</span>
          ...
          <p style={{ fontSize: '10px', color: 'var(--color-text-tertiary)', marginTop: '8px' }}>
            Critério de seleção: Evento de maior relevância técnica com município, empresa, fase e CAPEX homologados.
          </p>
