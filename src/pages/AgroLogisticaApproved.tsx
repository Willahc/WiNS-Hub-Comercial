...
type MapResponse = { status: string; items: Municipality[]; returned: number; total: number; aggregation: string; limit: number };
...
const coverageLabel: Record<CoverageStatus, string> = {
  COBERTURA_CONHECIDA_ALTA: 'Concentração conhecida alta',
  COBERTURA_CONHECIDA_MEDIA: 'Concentração conhecida média',
  COBERTURA_CONHECIDA_BAIXA: 'Concentração conhecida baixa',
  DADOS_INSUFICIENTES: 'Dados insuficientes',
};
const colors: Record<CoverageStatus, string> = {
  COBERTURA_CONHECIDA_ALTA: '#22C55E',
  COBERTURA_CONHECIDA_MEDIA: '#F59E0B',
  COBERTURA_CONHECIDA_BAIXA: '#F97316',
  DADOS_INSUFICIENTES: '#64748B',
};

const isWithinBrazil = (lat?: number, lng?: number): boolean =>
...
export default function AgroLogisticaApproved() {
  ...
  const [sort, setSort] = useState('transporters');
  ...
  const load = useCallback(async () => {
    ...
    if (mapResult.status === 'fulfilled') setMapData(mapResult.value.data); else setMapError('O mapa agregado não pôde ser consultado.');
    setLoading(false);
  }, [uf, query, coverage, page, sort, order]);
  useEffect(() => { load(); }, [load]);

  const validMapItems = (mapData?.items ?? []).filter(x => isWithinBrazil(x.latitude, x.longitude));
  ...
  >
    <section style={{ ...panel, background: 'rgba(59,130,246,.08)', borderColor: 'rgba(59,130,246,.35)', display: 'flex', gap: 10 }}>
      ...
      <div>
        <strong>Camada canônica disponível parcialmente</strong>
        <div style={{ fontSize: 12, color: '#CBD5E1', marginTop: 4 }}>
          O RNTRC nacional foi localizado, mas ainda não integra o contrato canônico desta página. A cobertura abaixo usa somente a camada já promovida e acessível.
        </div>
      </div>
    </section>
    ...
    <section aria-label="Resumo de cobertura" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: 12 }}>
      {cards.map(([label, value, Icon]) => (
        <article key={label} style={panel}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, marginTop: 8 }}>{fmt(value)}</div>
          <div style={{ fontSize: 11, color: '#94A3B8' }}>{label}</div>
        </article>
      ))}
    </section>

    <section style={{ ...panel, background: 'rgba(15,23,42,.6)', fontSize: 12, color: '#CBD5E1' }}>
      <strong>Auditoria de completude do RNTRC por UF:</strong> A completude do RNTRC varia por UF na camada disponível (SC possui 100% de preenchimento, enquanto MG, MS e SP possuem 0% cadastrado na ingestão atual). A contagem zero reflete a camada e o vínculo disponíveis nesta versão, e não ausência oficial de transportadores habilitados pelo RNTRC nesses estados.
    </section>

    <section style={panel} aria-label="Mapa territorial">
      <h3 style={{ marginTop: 0 }}>Mapa territorial agregado — Recorte limitado</h3>
      <p style={{ fontSize: 12, color: '#94A3B8' }}>
        Amostra agregada limitada a coordenadas válidas dentro do território nacional na camada disponível.
      </p>
      <div style={{ fontSize: 11, color: '#94A3B8', marginBottom: 8 }}>
        Exibindo {fmt(mapData?.returned ?? validMapItems.length)} municípios agregados com coordenadas válidas no Brasil (de {fmt(mapData?.total ?? municipal?.total ?? 0)} municípios no recorte).
      </div>
      {mapError ? (
        <InlineError text={mapError} retry={load} />
      ) : (
        <MapContainer
          center={[-14.2, -51.9]}
          zoom={4}
        ...
        >
          ...
          {validMapItems.map(x => (
            <CircleMarker
              key={`${x.uf}-${x.municipio}`}
              center={[Number(x.latitude), Number(x.longitude)]}
              radius={Math.min(18, 4 + Math.log10(1 + x.transporters) * 2)}
              pathOptions={{
                color: colors[x.coverage_status],
                fillColor: colors[x.coverage_status],
                fillOpacity: 0.65,
              }}
            >
              <Tooltip>
                <b>{x.municipio}/{x.uf}</b>
                <br />Transportadoras: {fmt(x.transporters)}
                <br />Geocodificadas: {fmt(x.geocoded)}
                <br />Rebanho: {fmt(x.livestock)}
                <br />{coverageLabel[x.coverage_status]}
              </Tooltip>
            </CircleMarker>
          ))}
        </MapContainer>
      ...
      <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 8 }}>
        🟢 Alta · 🟡 Média · 🟠 Baixa · ⚪ Dados insuficientes — sempre “concentração conhecida na camada disponível”.
      </div>
    </section>
    ...
    <section style={panel} aria-label="Tabela municipal">
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        <select
          ...
          value={coverage}
          onChange={e => { setCoverage(e.target.value); setPage(1); }}
        >
        ...
        <select aria-label="Ordenar" value={sort} onChange={e => setSort(e.target.value)}>
          <option value="transporters">Transportadoras</option>
          <option value="with_rntrc">Com RNTRC</option>
          <option value="geocoded">Geocodificadas</option>
          <option value="municipio">Município</option>
        </select>
        <button onClick={() => setOrder(v => v === 'desc' ? 'asc' : 'desc')}>
          {order === 'desc' ? 'Maior primeiro' : 'Menor primeiro'}
        </button>
      </div>
      {tableError ? (
        <InlineError text={tableError} retry={load} />
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
            <thead>
              <tr>
                {['Município', 'Operadores', 'RNTRC', 'Geo', 'Contatos', 'Registros calculados', 'Rebanho', 'Classificação'].map(h => (
                  <th key={h} style={{ textAlign: 'left', padding: 8, borderBottom: '1px solid var(--border-default)' }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {municipal?.items.map(row => (
                <tr key={`${row.uf}-${row.municipio}`}>
                  <td style={{ padding: 8 }}>
                    <b>{row.municipio}/{row.uf}</b>
                    <small style={{ display: 'block', color: '#64748B' }}>
                      IBGE {row.codigo_ibge ?? 'não ligado'}
                    </small>
                  </td>
                  <td>{fmt(row.transporters)}</td>
                  <td>{fmt(row.with_rntrc)}</td>
                  <td>{fmt(row.geocoded)}</td>
                  <td>{fmt(row.institutional_contacts)}</td>
                  <td>{fmt(row.operational_records)}</td>
                  <td>{fmt(row.livestock)}</td>
                  <td>{coverageLabel[row.coverage_status]}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 12, alignItems: 'center' }}>
        <span style={{ fontSize: 11 }}>{fmt(municipal?.total)} municípios no recorte</span>
        <div>
          <button disabled={page <= 1} onClick={() => setPage(p => p - 1)}>Anterior</button>
          <span style={{ margin: '0 10px' }}>{page} / {municipal?.pages || 0}</span>
          <button disabled={!municipal || page >= municipal.pages} onClick={() => setPage(p => p + 1)}>Próxima</button>
        </div>
      </div>
      <p style={{ fontSize: 10, color: '#64748B' }}>
        Ligação territorial declarada por nome normalizado + UF ou código IBGE exato, sem fuzzy matching silencioso. Proveniência por linha: log.transportadora, log.match, SICAR/CAR, IBGE e IBGE PPM. A contagem CAR municipal permanece oculta da tabela por não ser calculável na meta de desempenho desta camada.
      </p>
    </section>

    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 12 }}>
      <article style={panel}>
        <h3>Fontes da camada</h3>
        {[
          'log.transportadora — operadores e cobertura territorial',
          'log.match — registros logísticos previamente calculados',
          'SICAR/CAR — propriedades rurais',
          'IBGE — municípios',
          'IBGE PPM — pecuária municipal',
        ].map(x => (
          <p key={x} style={{ fontSize: 12 }}>✓ {x}</p>
        ))}
      </article>
      <article style={{ ...panel, borderColor: 'rgba(245,158,11,.35)' }}>
        <h3><AlertTriangle size={17} /> Infraestrutura não integrada</h3>
        {['CONAB', 'Armazéns', 'Capacidade estática', 'Terminais', 'Portos'].map(x => (
          <p key={x} style={{ fontSize: 12 }}>— {x}: indisponível no contrato canônico</p>
        ))}
      </article>
    </section>

    <section style={{ ...panel, fontSize: 11, color: '#94A3B8' }}>
      <strong>Limitações de uso</strong>
      {summary?.limitations.map(x => (
        <p key={x}>• {x}</p>
      ))}
      <p>
        Os registros de <code>log.match</code> são resultados previamente calculados; não comprovam relação comercial, frete aberto, viagem atual ou disponibilidade operacional.
      </p>
    </section>
  </AgroPageShell>;
}
...
function InlineError({ text, retry }: { text: string; retry: () => void }) {
  return (
    <div role="alert" style={{ padding: 16, color: '#FCA5A5' }}>
      {text} <button onClick={retry}>Tentar novamente</button>
    </div>
  );
}
